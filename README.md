...
getgenv().controllers = getgenv().controllers or {
    knifeLocked = false,
    gunLocked = false,
    toolsLocked = false,
    gunCooldown = 0,
...
local function getKnifeControls()
    ...
    if not knifeControl then return nil end
...
local function getControlButtons()
    ...
    if not knifeControl then return nil, nil, nil, nil end
...
local function throwKnife(tool, targetPosition, isManualActivation)
    if getgenv().controllers.knifeLocked or getgenv().controllers.toolsLocked then return end
